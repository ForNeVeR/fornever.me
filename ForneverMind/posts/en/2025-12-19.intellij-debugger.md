...
### Run Configuration
Now, we need to implement a run configuration to be able to actually run the user program in the IDE. Run configurations is a vast topic better discussed at [the documentation][intellij.run-configurations], so here I'll only describe the relevant parts of the system.
...
2. An implementation of the `RunConfiguration` interface that will be produced by the factory:
   ...
   ```kotlin
   class LuaFileRunConfiguration(…) : LocatableConfigurationBase<Element>(…) {
   ...
   ```
3. An implementation of the `RunProfileState` interface that contains the code that actually runs the process in the PTY mode:
   ...
   ```
4. For the user's convenience, to be able to use the **Run Current File** function, and run `.lua` files from the context menu, implement and register an implementation of the `RunConfigurationProducer` interface:
